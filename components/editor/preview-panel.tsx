...
export function PreviewPanel({ files, onError, isStreaming, onFilesChange, streamingContent = "" }: PreviewPanelProps) {
  ...
  useEffect(() => {
    const handleMessage = (e: MessageEvent) => {
      ...
      if (e.data && e.data.type === 'VISUAL_EDIT_IMAGE') {
        window.dispatchEvent(new CustomEvent("webagt:edit-product-image", {
          detail: { imageSrc: e.data.oldSrcResolved || e.data.oldSrc },
        }));
      }
  ...
  return (
    <div className="sandpack-stretch h-full w-full relative">
      ...
      {/* Visual Edit Mode Overlay Indicator — removed, button label is sufficient */}
